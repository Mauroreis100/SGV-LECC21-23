	...
	public boolean verificacaoQuantidade(int id, Vector lista, int quantidade) {
		...
		if (posicao != -1) {
			...
			if (quantidade > encontrado.getQtd()) { // Quantidade pretendida acima da que está no stock
				System.out.println("A quantidade pretendida de " + encontrado.getNome()
						+ " não esta disponível, apenas tem " + encontrado.getQtd());
				return false;
			} else if (encontrado.getQtd() < 0) {// ...
				System.out.println(
						"O produto" + encontrado.getNome() + " que esta a tentar vender não tem disponibilidade");
				return false;
